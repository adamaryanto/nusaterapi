...
@section('content')
    @php
        ...
        foreach ($allServices as $srv) {
            $serviceSlugMap[strtolower(trim($srv->name))] = $srv->slug;
        }
    @endphp
                    ...
                    <div class="flex justify-between items-center pb-4 border-b border-gray-100 mb-5">
                        <div class="flex items-center space-x-3 text-xs font-semibold">
                            ...
                            @if($booking->status === 'Menunggu Pembayaran')
                                ...
                                @if($remainingSeconds > 0)
                                    <span class="text-rose-600 font-bold flex items-center space-x-1 countdown-timer bg-rose-50 px-2 py-0.5 rounded border border-rose-100 text-[10px] tracking-wide" data-seconds="{{ $remainingSeconds }}">
                        ...
                        <div class="flex items-center space-x-2">
                            @if($booking->status === 'Selesai')
                                ...
                                @php
                                    $baseName = strtolower(trim(explode(' (', $booking->service_name)[0]));
                                    $bookingSlug = $serviceSlugMap[$baseName] ?? null;
                                @endphp
                            ...
                            @elseif($booking->status === 'Menunggu Pembayaran')
                                <button onclick="openPaymentModalForBooking(this, '{{ $booking->id }}')" class="px-4 py-2 text-xs font-bold rounded-lg transition shadow-sm bg-[#0f172a] text-white hover:bg-slate-800 inline-block focus:outline-none">
                                    Bayar Sekarang
                                </button>
    ...
    <script>
        ...
        function handleAction(orderId, actionType) {
            if (actionType === "Beri Ulasan") {
                window.location.href = `/riwayat-pesanan/ulasan/${orderId}`;
